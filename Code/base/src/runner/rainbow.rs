...
impl Rainbow {
    ...
    /// # Parameters
    /// 
    /// * `rainbow_length` - The length of time it takes for the rainbow to
    ///                      fully cycle through all the LEDs.
    /// * `saturation` - The saturation to be used for all colors generated by
    ///                  HSV values.
    /// * `value` - The value to be used for all colors generated by HSV values.
    /// * `arc` - The amount of the arc between hue 0 and 360 that is displayed
    ///           across all the LEDs.
    /// * `step` - The number of LEDs in a row that keep the same color before
    ///            moving on to the next color. E.g if a step of 1 yields an LED
    ///            array of \[1, 2, 3, 4\], then a step of 2 yields an array of
    ///            \[1, 1, 2, 2\].
